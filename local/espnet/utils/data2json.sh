...
Usage: $0 <data-dir> <dict> <cls_dict>
e.g. $0 data/train data/lang_1char/train_units.txt data/lang_1char/cls_units.txt
Options:
...
# # 4. Create scp files for cls outputs

if [ -n "${clsbpecode}" ]; then
    if [ ${multilingual} = true ]; then
        # remove a space before the language ID
        paste -d " " <(awk '{print $1}' ${cls}) <(cut -f 2- -d" " ${cls} \
            | spm_encode --model=${clsbpecode} --output_format=piece | cut -f 2- -d" ") \
            > ${tmpdir}/output/cls_token.scp
    else
        paste -d " " <(awk '{print $1}' ${cls}) <(cut -f 2- -d" " ${cls} \
            | spm_encode --model=${clsbpecode} --output_format=piece) \
            > ${tmpdir}/output/cls_token.scp
    fi
elif [ -n "${nlsyms}" ]; then
    text2token.py -s 1 -n 1 -l ${nlsyms} ${cls} --trans_type ${trans_type} > ${tmpdir}/output/cls_token.scp
else
    text2token.py -s 1 -n 1 ${cls} --trans_type ${trans_type} > ${tmpdir}/output/cls_token.scp
fi
< ${tmpdir}/output/cls_token.scp utils/sym2int.pl --map-oov ${oov} -f 2- ${cls_dic} > ${tmpdir}/output/cls_tokenid.scp
# +2 comes from CTC blank and EOS
vocsize=$(tail -n 1 ${cls_dic} | awk '{print $2}')
odim=$(echo "$vocsize + 2" | bc)
< ${tmpdir}/output/cls_tokenid.scp awk -v odim=${odim} '{print $1 " " NF-1 "," odim}' > ${tmpdir}/output/cls_shape.scp

cat ${cls} > ${tmpdir}/output/cls.scp
...
for intype in ${intypes}; do
    ...
    for x in "${tmpdir}/${intype}"/*.scp; do
        ...
        if [ ${k} = shape ]; then
            opts+="shape:${x}:shape "
        elif [ ${k} = cls_shape ]; then
            opts+="cls_shape:${x}:shape "
        else
            opts+="${k}:${x} "
        fi
    done
