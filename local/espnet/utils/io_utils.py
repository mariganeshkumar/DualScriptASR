...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        z_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        ...
        for uttid, info in batch:
            ...
            if self.mode != "asr":
                logging.error(
                "not configured for modes other than ASR "
                )
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output"]): 
                    x = np.fromiter(
                        map(int, inp["cls_tokenid"].split()), dtype=np.int64
                    )
                    z_feats_dict.setdefault('target0.5', []).append(x)


        
        return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, z_feats_dict, y_feats_dict, uttid_list
            )
    ...
    def _create_batch_asr(self, x_feats_dict, z_feats_dict, y_feats_dict, uttid_list):
        ...
        if self.load_output:
            ...
            zs = list(z_feats_dict.values())
            assert len(xs[0]) == len(ys[0]), (len(xs[0]), len(ys[0]))

            assert len(xs[0]) == len(zs[0]), (len(xs[0]), len(zs[0]))
        ...
        if self.load_output:
            ...
            zs = [[z[i] for i in nonzero_sorted_idx] for z in zs]
            z_names = list(z_feats_dict.keys())            

            # Keeping x_name and y_name, e.g. input1, for future extension
            return_batch = OrderedDict(
                [
                    *[(x_name, x) for x_name, x in zip(x_names, xs)],
                    *[(z_name, z) for z_name, z in zip(z_names, zs)],
