...
from local.espnet.utils.io_utils import LoadInputsAndTargets
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        zs_pad = pad_list(
            [
                torch.from_numpy(
                    np.array(z[0][:]) if isinstance(z, tuple) else z
                ).long()
                for z in zs
            ],
            self.ignore_id,
        ...
        return xs_pad, ilens, zs_pad, ys_pad
...
def train(args):
    ...
    intermediate_odim = int(valid_json[utts[0]]["output"][0]["cls_shape"][-1])
    odim = int(valid_json[utts[0]]["output"][0]["shape"][-1])
    for i in range(args.num_encs):
        logging.info("stream{}: input dims : {}".format(i + 1, idim_list[i]))
    logging.info("#intermediate output dims: " + str(intermediate_odim))
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        model = model_class(
            idim_list[0] if args.num_encs == 1 else idim_list, intermediate_odim, odim, args
        )
    ...
    cpu_model = cpu_model.to(device=torch.device("cpu"), dtype=dtype)
    ...
    trainer.extend(
        extensions.PlotReport(
            [
                ...
                "main/loss_ctc_text",
                "main/loss_ctc_cls",
                "validation/main/loss_ctc_text",
                "validation/main/loss_ctc_cls",
                "main/loss_att_text",
                "main/loss_att_cls",
                "validation/main/loss_att_text",
                "validation/main/loss_att_cls",
            ]
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/acc_text", "main/acc_cls", "validation/main/acc_text", "validation/main/acc_cls"], "epoch", file_name="acc.png"
        )
    )
    trainer.extend(
        extensions.PlotReport(
            ["main/cer_ctc", "validation/main/cer_ctc_text"]
            + ([] if args.num_encs == 1 else report_keys_loss_ctc),
            "epoch",
            file_name="cer.png",
        )
    ...
    if mtl_mode not in ["ctc", "transducer", "transformer_transducer"]:
        trainer.extend(
            ...
            trigger=training.triggers.MaxValueTrigger("validation/main/acc_text"),
        )
    ...
    report_keys = [
        ...
        "validation/main/acc_text",
        "main/acc_cls",
        "validation/main/acc_cls",
        "main/cer_text",
        "validation/main/cer_text",
        "main/cer_ctc",
    ...
    if args.report_cer:
        report_keys.append("validation/main/cer_text")
    if args.report_wer:
        report_keys.append("validation/main/wer_text")
    trainer.extend(
    ...
    # self.iteration += 1 # Increase may result in early report,
    # which is done in other place automatically.
    writer.add_graph(cpu_model, batch)
    writer.close()
    trainer.run()
