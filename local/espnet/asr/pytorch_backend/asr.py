...
def train(args):
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        model = load_trained_modules(idim_list[0], intermediate_odim, odim, args)
    else:
    ...
    with open(model_conf, "wb") as f:
        ...
        f.write(
            json.dumps(
                (idim_list[0] if args.num_encs == 1 else idim_list, intermediate_odim, odim, vars(args)),
                indent=4,
                ensure_ascii=False,
                sort_keys=True,
            ).encode("utf_8")
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/acc", "main/acc_cls", "validation/main/acc", "validation/main/acc_cls"], "epoch", file_name="acc.png"
        )
    )
    trainer.extend(
        extensions.PlotReport(
