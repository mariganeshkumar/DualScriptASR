...
from local.espnet.asr.asr_utils import get_model_conf
...
def load_trained_model(model_path, training=True):
    ...
    """
    idim, inter_dim, odim, train_args = get_model_conf(
    ...
    if "transducer" in model_module:
        model = model_class(idim, inter_dim, odim, train_args, training)
    else:
        model = model_class(idim, inter_dim, odim, train_args)
