...
class ASRInterface:
    ...
    @classmethod
    def build(cls, idim: int, inter_dim:int, odim: int, **kwargs):
        """Initialize this class with python-level args.

        Args:
            idim (int): The number of an input feature dim.
            inter_dim (int) : The number of intermediate output vocab
    ...
    def forward(self, xs, ilens, zs, ys):
        ...
        :param ys:
            ...
            For chainer, list of source sequences chainer.Variable
        :param ys:
        	For pytorch, batch of padded source sequences torch.Tensor (B, Lmax_inter) #intermediate respresntation
        :return: loss value
    ...
    def calculate_all_attentions(self, xs, ilens, ys, zs):
        """Caluculate attention.

        :param list xs_pad: list of padded input sequences [(T1, idim), (T2, idim), ...]
        :param ndarray ilens: batch of lengths of input sequences (B)
        :param list ys: list of character id sequence tensor [(L1), (L2), (L3), ...]
        :param list zs: list of intermediate character id sequence tensor [(M1), (M2), (M3), ...]
        :return: attention weights (B, Lmax, Tmax)
        :rtype: float ndarray
        """
        raise NotImplementedError("calculate_all_attentions method is not implemented")

    def calculate_all_ctc_probs(self, xs, ilens, ys, zs):
...
predefined_asr = {
    "pytorch": {
        ...
        "transformer": "local.espnet.nets.pytorch_backend.e2e_asr_transformer:E2E",
    },
}
